...
class SplashScreen : AppCompatActivity() {
    ...
    var viewHandler: Handler? = null
    var loadingTextViewAnimator: TextViewDotAnimator? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_slash_screen)
        viewHandler = Handler()
        loadingTextViewAnimator = TextViewDotAnimator(tv_loading, viewHandler);
        viewHandler?.post(loadingTextViewAnimator);
        ...
        async(UI) {
            ...
            if (permission.await()) {
                delay(2000)
                viewHandler?.removeCallbacks(loadingTextViewAnimator)
